 ...
 */
@Service
public class ServiceBean extends com.panforge.demeter.service.Service<PageCursorByPageNumber> {
  private static final Logger LOG = LoggerFactory.getLogger(ServiceBean.class);

  @Autowired 
  public ServiceBean(ConfigService config, ContentProvider<PageCursorByPageNumber> repo, TokenManager tokenManager, @Value("${batchSize}") int batchSize) {
